...
const PetAdopterNotificationsScreen = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      try {
        ...
        if (user) {
            console.log('user');
          const notificationsQuery = query(collection(db, "notifications"),where("receiver_id", "==", user.id), where("action","==", "matched"));
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => navigateToUserProfile(item.senderProfile.uid, item.pet_id)}>
      <View style={styles.card}>
        ...
        <Text>{`${item.senderProfile.displayName} matched with you. `}</Text>
...
export default PetAdopterNotificationsScreen;
